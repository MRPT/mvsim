...
void mvsim::parse_xmlnode_attribs(
	...
	const std::map<std::string, std::string>& variableNamesValues, const char* functionNameContext)
{
	for (const auto& param : params)
	{
		if (auto attr = xml_node.first_attribute(param.first.c_str()); attr && attr->value())
		{
			param.second.parse(
				attr->value(), attr->name(), variableNamesValues, functionNameContext);
		}
	}
}
	...
	mrpt::system::COutputLogger* logger)
{
	...
	{
		bool recognized =
			parse_xmlnode_as_param(*node, params, variableNamesValues, functionNameContext);
		if (!recognized && logger)
		{
			logger->logFmt(
				mrpt::system::LVL_WARN, "Unrecognized tag '<%s>' in %s", node->name(),
...
mrpt::math::TPose2D mvsim::parseXYPHI(
	const std::string& sOrg, bool allow_missing_angle, double default_angle_radians,
	const std::map<std::string, std::string>& variableNamesValues)
{
	mrpt::math::TPose2D v;
...
std::tuple<std::shared_ptr<rapidxml::xml_document<>>, rapidxml::xml_node<>*>
	mvsim::readXmlTextAndGetRoot(const std::string& xmlData, const std::string& pathToFile)
{
	using namespace rapidxml;
	...
	catch (rapidxml::parse_error& e)
	{
		...
		throw std::runtime_error(
			mrpt::format("XML parse error (Line %u): %s", static_cast<unsigned>(line), e.what()));
	}

	// Sanity check:
	xml_node<>* root = xml->first_node();
	ASSERTMSG_(root, "XML parse error: No root node found (empty file '"s + pathToFile + "'?)"s);
	return {xml, root};
}
	...
	const std::set<std::string>& varsRetain)
{
	...
	childVariables["MVSIM_CURRENT_FILE_DIRECTORY"] = mrpt::system::toAbsolutePath(
		mrpt::system::extractFileDirectory(pathToFile), false /*canonical*/);

	xmlDoc->documentData =
		mvsim::parse_variables(mrpt::io::file_get_contents(pathToFile), childVariables, varsRetain);
...
static std::string parseVars(
	const std::string& text, const std::map<std::string, std::string>& variables,
	const std::set<std::string>& varsRetain, const size_t searchStartPos = 0)
{
	...
	else
	{
		...
		{
			THROW_EXCEPTION_FMT("mvsim::parseVars(): Undefined variable: ${%s}", varname.c_str());
		}
	}

	return parseVars(pre + varvalue + post.substr(post_end + 1), variables, varsRetain);
	MRPT_TRY_END
}
