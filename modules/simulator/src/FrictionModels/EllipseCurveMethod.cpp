	...
	const FrictionBase::TFrictionInput& input) const
{
    ...
    mrpt::math::TPoint2D Center_of_mass = myVehicle_.getChassisCenterOfMass(); // ¿esta bien este codigo?// ¿esta bien este codigo?
    const double h = 0.40; // altura del centro de gravedad provisional
 	const size_t nW = myVehicle_.getNumWheels();    //esta linea ya existe en VehicleBase.ccp linea 568
    ...
    const double a1 = std::abs(pos[3].x), a2 = std::abs(pos[0].x); // distancia centro de gravedad a ejes
    const double l = a1 + a2; // distancia entre ejes
    const double Axf = std::abs(pos[2].y)+std::abs(pos[3].y), Axr = std::abs(pos[0].y)+std::abs(pos[1].y); // longitud ejes
    ASSERT_(Axf > 0);
    ASSERT_(Axr > 0);

    const mrpt::math::TPoint3D_<double> linAccLocal = myVehicle_.getLinearAcceleration(); // ¿Está bien?
    ...
    //tengo que cambiar la realcion de cada ecuacion ya que no coincide con la rueda

    if (wheel == 0) {
    double Fz = (m / (l * Axf * gravity)) * (a2 * gravity - h * (linAccLocal.x - w * vel.vy)) * (std::abs(pos[1].y) * gravity - h * (linAccLocal.y + w * vel.vx)); 
    }else if (wheel == 1) {
    double Fz = (m / (l * Axf * gravity)) * (a2 * gravity - h * (linAccLocal.x - w * vel.vy)) * (std::abs(pos[0].y) * gravity + h * (linAccLocal.y + w * vel.vx)); 
    }else if (wheel == 2) {
    double Fz = (m / (l * Axr * gravity)) * (a1 * gravity + h * (linAccLocal.x - w * vel.vy)) * (std::abs(pos[3].y) * gravity - h * (linAccLocal.y + w * vel.vx)); 
    }else if (wheel == 3) {
    double Fz = (m / (l * Axr * gravity)) * (a1 * gravity + h * (linAccLocal.x - w * vel.vy)) * (std::abs(pos[2].y) * gravity + h * (linAccLocal.y + w * vel.vx)); 
    } else {
