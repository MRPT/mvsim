...
GroundGrid::GroundGrid(World*parent,const rapidxml::xml_node<char> *root) :
	...
	m_is_floating(true),
	m_x_min(-25.0),m_x_max(25.0),m_y_min(-25.0),m_y_max(25.0),
	m_interval(5.0),
...
void GroundGrid::loadConfigFrom(const rapidxml::xml_node<char> *root)
{
	...
	// If a vehicle name is given, setting "is_floating=true" by the user is optional:
...
void GroundGrid::gui_update( mrpt::opengl::COpenGLScene &scene)
{
	...
	if (!m_gl_groundgrid)
	{
    m_gl_groundgrid =  mrpt::make_aligned_shared<mrpt::opengl::CGridPlaneXY>();
	...
	if (m_is_floating)
	{
		...
		World::TListVehicles::const_iterator it_veh = vehs.find(m_float_center_at_vehicle_name);
		// not found -> error:
		if (!m_float_center_at_vehicle_name.empty() && it_veh==vehs.end())
			throw std::runtime_error(mrpt::format("[GroundGrid] *ERROR* Cannot find vehicle named '%s' to focus on.",m_float_center_at_vehicle_name.c_str()));
		
		// If the user didn't specify any name, assume it's the first vehicle, or ignore it if none exists:
		if (it_veh==vehs.end()) 
			it_veh=vehs.begin();
	...
	center_offset.x = m_interval* ::floor(std::abs(center_offset.x)/m_interval) * (center_offset.x<0 ? -1. : 1.);
	center_offset.y = m_interval* ::floor(std::abs(center_offset.y)/m_interval) * (center_offset.y<0 ? -1. : 1.);
