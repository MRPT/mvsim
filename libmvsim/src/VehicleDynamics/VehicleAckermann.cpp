...
void DynamicsAckermann::dynamics_load_params_from_xml(const rapidxml::xml_node<char> *xml_node)
{
	...
	if (xml_chassis)
	{
		...
		if (xml_shape)
			mvsim::parse_xmlnode_shape(*xml_shape, m_chassis_poly, "[DynamicsAckermann::dynamics_load_params_from_xml]");
	}
...
void DynamicsAckermann::invoke_motor_controllers(const TSimulContext &context, std::vector<double> &out_torque_per_wheel)
{
	...
	if (m_controller)
	{
		...
		computeFrontWheelAngles(
			co.steer_ang,
			m_wheels_info[WHEEL_FL].yaw,
			m_wheels_info[WHEEL_FR].yaw);
	}

}

void DynamicsAckermann::computeFrontWheelAngles(const double desired_equiv_steer_ang, double &out_fl_ang,double &out_fr_ang) const
{
	...
	const double delta= b2Clamp( std::abs(desired_equiv_steer_ang), 0.0, m_max_steer_ang);
